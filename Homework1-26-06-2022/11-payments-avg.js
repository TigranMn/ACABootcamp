...
function getPaymentsAvg(paymentsList) {
  let paymentsAvg = getPaymentsSum(paymentsList) / paymentsList.length;
  return paymentsAvg;
}
...
function getPaymentsSum(paymentsList) {
  ...
  for (let i = 0; i < paymentsList.length; i++) {
    paymentsSum += paymentsList[i].payedAmount;
  }
  return paymentsSum;
}
console.log(getPaymentsAvg(payments));
